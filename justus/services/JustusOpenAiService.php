...
class JustusOpenAiService
{
    ...
    public function sendMessage(JustusConversation $conversation, string $userMessage): array
    {
        ...
        $ragContext = $this->rag->buildContextFromChunks($chunks);

        $styleGuide = JustusStyleGuide::active();
        ...
        try {
            ...
            if (!$response->successful()) {
                ...
                SystemEvent::sistema(
                    'JUSTUS: Erro na chamada OpenAI',
                    'error',
                    ['conversation_id' => $conversation->id, 'http_status' => $response->status(), 'error' => $errorMsg],
                    $userId
                );
            ...
            SystemEvent::sistema(
                'JUSTUS: Exceção na chamada OpenAI',
                'error',
                ['conversation_id' => $conversation->id, 'exception' => $e->getMessage()],
                $userId
            );
