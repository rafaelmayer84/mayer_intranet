<?php $__env->startSection('title', 'Sincronização Unificada'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Gerenciamento centralizado de dados DataJuri + ESPO CRM</p>
        </div>
        <div class="flex items-center gap-2">
            <span class="text-xs text-gray-500">Última sync:</span>
            <span class="rounded bg-gray-100 px-2 py-1 text-xs font-medium dark:bg-gray-700"><?php echo e($lastSyncDataJuri); ?></span>
        </div>
    </div>
    ...
    <div id="content-dashboard" class="tab-content">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            <?php $__currentLoopData = $modulosDisponiveis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $mod): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="rounded-lg bg-white p-4 shadow dark:bg-gray-800 border-l-4
                <?php if($key == 'Pessoa'): ?> border-blue-500
                <?php elseif($key == 'Processo'): ?> border-purple-500
                <?php elseif($key == 'Fase'): ?> border-indigo-500
                <?php elseif($key == 'Movimento'): ?> border-green-500
                <?php elseif($key == 'Contrato'): ?> border-yellow-500
                <?php elseif($key == 'Atividade'): ?> border-orange-500
                <?php elseif($key == 'HoraTrabalhada'): ?> border-pink-500
                <?php elseif($key == 'OrdemServico'): ?> border-red-500
                <?php elseif($key == 'ContasReceber'): ?> border-teal-500
                <?php else: ?> border-gray-500
                <?php endif; ?>">
                <div class="flex items-center justify-between">
                    <span class="text-2xl"><?php echo e($mod['icon']); ?></span>
                    <span class="text-xs text-gray-400"><?php echo e($key); ?></span>
                </div>
                <p class="mt-2 text-xl font-bold text-gray-900 dark:text-white" id="count-<?php echo e($mod['table']); ?>">
                    <?php echo e(number_format($mod['count'], 0, ',', '.')); ?>

                </p>
                <p class="text-xs text-gray-600 dark:text-gray-400"><?php echo e($mod['label']); ?></p>
            </div>
        ...
        <div class="mt-6 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
            ...
            <div class="border-t pt-4 mt-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <?php $__currentLoopData = $modulosDisponiveis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $mod): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button onclick="syncModule('<?php echo e($key); ?>')"
                        ...
                        <span><?php echo e($mod['icon']); ?></span> <?php echo e($mod['label']); ?>
        ...
        <div class="mt-6 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">&#x1F517; ESPO CRM</h2>
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="text-xs text-gray-500">Leads</p>
                    <p class="text-xl font-bold"><?php echo e(number_format($leads, 0, ',', '.')); ?></p>
                </div>
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="text-xs text-gray-500">Contatos</p>
                    <p class="text-xl font-bold"><?php echo e(number_format($contacts, 0, ',', '.')); ?></p>
                </div>
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="text-xs text-gray-500">Contas</p>
                    <p class="text-xl font-bold"><?php echo e(number_format($contas, 0, ',', '.')); ?></p>
                </div>
            </div>
            <div class="flex flex-wrap gap-3 mb-4">
                <button id="btn-test-espocrm" class="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 flex items-center gap-2">
                    <span>&#x1F50C;</span> Testar Conexão
                </button>
                <button id="btn-sync-espocrm" class="rounded-lg bg-green-600 px-4 py-2 text-sm text-white hover:bg-green-700 flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y bg-white dark:bg-gray-800">
                        <?php $__empty_1 = true; $__currentLoopData = $historico; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $run): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-4 py-3 text-sm font-medium"><?php echo e($run->tipo); ?></td>
                            <td class="px-4 py-3 text-sm">
                                <span class="rounded-full px-2 py-0.5 text-xs
                                    <?php if($run->status === 'completed'): ?> bg-green-100 text-green-800
                                    <?php elseif($run->status === 'running'): ?> bg-blue-100 text-blue-800
                                    <?php elseif($run->status === 'failed'): ?> bg-red-100 text-red-800
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">
                                <?php if($run->registros_processados): ?>
                                    <?php echo e(number_format($run->registros_processados, 0, ',', '.')); ?>

                                    <?php if($run->registros_criados): ?> <span class="text-green-600">+<?php echo e($run->registros_criados); ?></span> <?php endif; ?>
                                    <?php if($run->registros_atualizados): ?> <span class="text-blue-600">~<?php echo e($run->registros_atualizados); ?></span> <?php endif; ?>
                                <?php else: ?>
                                    -
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-500"><?php echo e($run->started_at ? \Carbon\Carbon::parse($run->started_at)->format('d/m H:i') : '-'); ?></td>
                            <td class="px-4 py-3 text-sm text-gray-500"><?php echo e($run->duracao ? $run->duracao.'s' : '-'); ?></td>
                            <td class="px-4 py-3 text-sm text-gray-500 max-w-xs truncate" title="<?php echo e($run->mensagem); ?>"><?php echo e(Str::limit($run->mensagem, 50)); ?></td>
                        </tr>
...
async function refreshCounts() {
    try {
        const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.index")); ?>/counts', {
...
async function syncModule(modulo) {
    ...
    try {
        const r = await fetch(`<?php echo e(url('admin/sincronizacao-unificada/sync')); ?>/${modulo}`, {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btn-test-datajuri').addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.smoke-test")); ?>', {
                method: 'POST',
    ...
    const modulosParaSync = <?php echo json_encode(array_keys($modulosDisponiveis), 15, 512) ?>;
    ...
    document.getElementById('btn-sync-all').addEventListener('click', async () => {
        ...
        for (let i = 0; i < modulos.length; i++) {
            ...
            try {
                const r = await fetch('<?php echo e(url("admin/sincronizacao-unificada/sync")); ?>/' + mod, {
                    method: 'POST',
    ...
    document.getElementById('btn-reprocessar').addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.reprocessar-financeiro")); ?>', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
            });
            const d = await r.json();
    ...
    document.getElementById('btn-test-espocrm').addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.espocrm-test")); ?>', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
            });
            const d = await r.json();
    ...
    document.getElementById('btn-sync-espocrm').addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.espocrm-sync")); ?>', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
            });
            const d = await r.json();
    ...
    document.getElementById('btn-limpar-historico').addEventListener('click', async () => {
        ...
        try {
            const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.clear-history")); ?>', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
            });
            const d = await r.json();
...
async function carregarStats() {
    try {
        const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.classificacao.stats")); ?>');
        const d = await r.json();
...
async function carregarRegras(p = 1) {
    ...
    const url = new URL('<?php echo e(route("admin.sincronizacao-unificada.classificacao.index")); ?>');
...
async function salvarRegra() {
    ...
    try {
        const url = id
            ? '<?php echo e(url("admin/sincronizacao-unificada/classificacao")); ?>/' + id
            : '<?php echo e(route("admin.sincronizacao-unificada.classificacao.store")); ?>';
        const r = await fetch(url, {
            method: id ? 'PUT' : 'POST',
            headers: {
...
async function excluirRegra(id) {
    ...
    try {
        const r = await fetch('<?php echo e(url("admin/sincronizacao-unificada/classificacao")); ?>/' + id, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
        });
        const d = await r.json();
...
async function importarRegras() {
    ...
    try {
        const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.classificacao.importar")); ?>', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
        });
        const d = await r.json();
...
async function reclassificar() {
    ...
    try {
        const r = await fetch('<?php echo e(route("admin.sincronizacao-unificada.classificacao.reclassificar")); ?>', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }
        });
        const d = await r.json();
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u492856976/domains/mayeradvogados.adv.br/public_html/Intranet/resources/views/admin/sincronizacao-unificada/index.blade.php ENDPATH**/ ?>