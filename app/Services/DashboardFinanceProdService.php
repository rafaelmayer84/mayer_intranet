...
public function getLucratividadeByMonth(int $ano): array
{
    ...
    private function getResumoExecutivo(int $ano, int $mes): array
    {
        ...
        if (!$metaDespesasDb) {
            $metaDespesasAnual = $this->getMetaValueFromKpiTable("despesa_total_ano", 0, $ano);
            if ($metaDespesasAnual) {
                $metaDespesasDb = $metaDespesasAnual / 12;
            }
        }
    ...
    /**
     * Recupera meta específica da tabela kpi_monthly_targets
     */
    private function getMetaValueFromKpiTable(string $kpiKey, int $mes, int $ano): ?float
    {
        $meta = DB::table('kpi_monthly_targets')
            ->where('year', $ano)
            ->where('month', $mes)
            ->where('kpi_key', $kpiKey)
            ->first();
        
        if ($meta && $meta->target_value) {
            return (float) $meta->target_value;
        }
