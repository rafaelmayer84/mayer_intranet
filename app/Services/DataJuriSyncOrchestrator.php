...
class DataJuriSyncOrchestrator
{
    ...
    protected function upsertRecord(string $modulo, array $row, array $config): string
    {
        ...
        // =====================================================================
        // TRATAMENTO ESPECIAL: MÃ³dulo Pessoa (PATCH 07/02/2026)
        // Campos que precisam de conversÃ£o especial antes do upsert
        // =====================================================================
        if ($modulo === 'Pessoa') {
            // 1. is_cliente: API retorna "Sim"/"NÃ£o" no campo 'cliente'
            $clienteFlag = $row['cliente'] ?? '';
            $data['is_cliente'] = (mb_strtolower(trim($clienteFlag)) === 'sim') ? 1 : 0;

            // 2. tipo: limpar "Pessoa FÃ­sica - Cliente" â†’ "PF", "Pessoa JurÃ­dica" â†’ "PJ"
            $tipoPessoa = $data['tipo'] ?? '';
            if (stripos($tipoPessoa, 'FÃ­sica') !== false || stripos($tipoPessoa, 'sica') !== false) {
                $data['tipo'] = 'PF';
            } elseif (stripos($tipoPessoa, 'JurÃ­dica') !== false || stripos($tipoPessoa, 'dica') !== false) {
                $data['tipo'] = 'PJ';
            }
            ...
            // 4. telefone_normalizado: extrair apenas dÃ­gitos, adicionar DDI 55
            $fonePrincipal = !empty($tel) ? $tel : $cel;
            $data['telefone_normalizado'] = $this->normalizarTelefone($fonePrincipal);

            // 5. endereco: montar endereÃ§o completo a partir dos campos decompostos
            $partes = array_filter([
                $data['endereco_rua'] ?? '',
        ...
        // =====================================================================
        // TRATAMENTO ESPECIAL: MÃ³dulo Processo (FIX 07/02/2026)
        // ganho_causa: API retorna "Sim"/"NÃ£o", banco espera tinyint
        // =====================================================================
        if ($modulo === 'Processo') {
...
if ($existing) {
     ...
     * Ex: "(47) 99999-0001" â†’ "5547999990001"
     * Ex: "+55 47 99999-0001" â†’ "5547999990001"
     * Ex: "47999990001" â†’ "5547999990001"
     */
    protected function normalizarTelefone(?string $telefone): ?string
    {
        ...
        // Se comeÃ§a com 55 e tem 12-13 dÃ­gitos â†’ jÃ¡ tem DDI
        if (preg_match('/^55\d{10,11}$/', $digits)) {
            return $digits;
        }

        // Se tem 10-11 dÃ­gitos â†’ DDD + nÃºmero, adicionar DDI 55
        if (preg_match('/^\d{10,11}$/', $digits)) {
            return '55' . $digits;
        }

        // Se tem 8-9 dÃ­gitos â†’ sÃ³ nÃºmero sem DDD, nÃ£o conseguimos normalizar com confianÃ§a
        return null;
    }
