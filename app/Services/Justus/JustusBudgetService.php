...
class JustusBudgetService
{
    ...
    public function calculateCost(int $inputTokens, int $outputTokens, string $model = null): float
    {
        ...
        $allPricing = config('justus.pricing');
        $pricing = $allPricing[$model] ?? $allPricing['gpt-5.2'] ?? ['input_per_1m' => 1.75, 'output_per_1m' => 14.00];
        $usdBrl = config('justus.usd_brl', 5.80);

        $inputCostUsd = ($inputTokens / 1000000) * $pricing['input_per_1m'];
        $outputCostUsd = ($outputTokens / 1000000) * $pricing['output_per_1m'];
        $totalUsd = $inputCostUsd + $outputCostUsd;

        return round($totalUsd * $usdBrl, 4);
    }

    public function calculateCostUsd(int $inputTokens, int $outputTokens, string $model = null): float
    {
        $model = $model ?? config('justus.model_default');
        $allPricing = config('justus.pricing');
        $pricing = $allPricing[$model] ?? $allPricing['gpt-5.2'] ?? ['input_per_1m' => 1.75, 'output_per_1m' => 14.00];
        ...
        return round($inputCostUsd + $outputCostUsd, 6);
    }
}
