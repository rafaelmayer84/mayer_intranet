                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @endif
                                    @if($usuario->id !== auth()->id() && auth()->user()->role === 'admin')
                                        <form action="{{ route('admin.usuarios.destroy', $usuario) }}" method="POST" class="inline"
                                              onsubmit="return confirm('Excluir permanentemente ' + '{{ $usuario->name }}' + '? Esta ação NÃO pode ser desfeita.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-400 hover:text-red-600" title="Excluir permanentemente">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
