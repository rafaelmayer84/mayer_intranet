        ...
        <div class="mt-6 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    ...
                    <p class="text-xl font-bold">{{ number_format($leads, 0, ',', '.') }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="text-xs text-gray-500">Contatos</p>
                    <p class="text-xl font-bold">{{ number_format($contacts, 0, ',', '.') }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="text-xs text-gray-500">Contas</p>
                    <p class="text-xl font-bold">{{ number_format($contas, 0, ',', '.') }}</p>
                </div>
            </div>
            <div class="flex flex-wrap gap-3 mb-4">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btn-sync-all').addEventListener('click', async () => {
        ...
        if (!confirm('Sincronizar TODOS os módulos? Isso pode levar vários minutos.')) return;

        syncInProgress = true;
        addLog('&#x1F680; Iniciando sincronização completa...', 'info');
        showProgress('Sincronizando todos os módulos...', 5);

        try {
            const r = await fetch('{{ route("admin.sincronizacao-unificada.sync-all") }}', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
            });
            const d = await r.json();
