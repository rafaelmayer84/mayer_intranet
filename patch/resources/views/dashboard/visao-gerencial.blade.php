    ...
    function renderTables(d) {
            ...
            tbodyAtr.innerHTML = atrasos.map(function (c) {
                return (
                    '<tr>' +
                        ...
                        '<td class="px-3 py-2">' + escapeHtml(c.dias_atraso) + '</td>' +
                    '</tr>'
                );
            }).join('');
        }
        ...
        if (tbodyComp) {
            ...
            var metrics = [
                ...
                { key: 'margem', label: 'Margem (%)', isPct: true }
            ];

            tbodyComp.innerHTML = metrics.map(function (m) {
                var v2 = asNumber(m2[m.key]);
                var v1 = asNumber(m1[m.key]);
                var va = asNumber(atual[m.key]);
                var tr = trend[m.key];

                function show(v) { return m.isPct ? (Math.round(v * 10) / 10).toFixed(1) + '%' : fmtBRL(v); }

                return (
                    '<tr>' +
                        '<td class="px-3 py-2">' + escapeHtml(m.label) + '</td>' +
                        '<td class="px-3 py-2">' + show(v2) + '</td>' +
                        '<td class="px-3 py-2">' + show(v1) + '</td>' +
                        '<td class="px-3 py-2">' + show(va) + '</td>' +
                        '<td class="px-3 py-2">' + escapeHtml(tr || '') + '</td>' +
                    '</tr>'
                );
            }).join('');
        }
